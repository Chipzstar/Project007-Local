...
import img from '../../../assets/images/drawable-xxxhdpi/car_set_up.png';

export default class SetUpTile extends Component {
    constructor(props) {
        super(props);
        this.state = {
            loading: true,
...
//The code that is called when the component is first mounted. Use it to setup the component and load the image files
    ...
    async getAndLoadFirebaseUrl() {
        if (this.state.mounted) {
            const ref = await firebase.storage().ref('TrackModeScreenTiles/car_set_up.png');
