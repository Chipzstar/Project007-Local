...
import img from '../../../assets/images/drawable-xxxhdpi/telemetry.png';

export default class TelemetryTile extends Component {
    constructor(props) {
        super(props);
        this.state = {
            loading: true,
...
//The code that is called when the component is first mounted. Use it to setup the component and load the image files
    ...
    async getAndLoadFirebaseUrl() {
        if (this.state.mounted) {
            const ref = await firebase.storage().ref('TrackModeScreenTiles/telemetry.png');
