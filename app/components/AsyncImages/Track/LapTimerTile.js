...
import img from '../../../assets/images/drawable-xxxhdpi/lap_timer.png';

export default class LapTimerTile extends Component {
    constructor(props) {
        super(props);
        this.state = {
            loading: true,
...
//The code that is called when the component is first mounted. Use it to setup the component and load the image files
    ...
    async getAndLoadFirebaseUrl() {
        if (this.state.mounted) {
            const ref = await firebase.storage().ref('TrackModeScreenTiles/lap_timer.png');
