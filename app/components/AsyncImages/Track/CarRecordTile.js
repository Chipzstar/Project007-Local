...
import img from '../../../assets/images/drawable-xxxhdpi/in_car_video.png';

export default class CarRecordTile extends Component {
    constructor(props) {
        super(props);
        this.state = {
            loading: true,
...
//The code that is called when the component is first mounted. Use it to setup the component and load the image files
    ...
    async getAndLoadFirebaseUrl() {
        if (this.state.mounted) {
            const ref = await firebase.storage().ref('TrackModeScreenTiles/in_car_video.png');
